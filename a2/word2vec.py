...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs = np.matmul(np.expand_dims(y_hat,1),np.expand_dims(centerWordVec,0))
...
def negSamplingLossAndGradient(
    ...
    for i in negSampleWordIndices:
        mul_kc = -np.dot(outsideVectors[i],centerWordVec)
        sum1 += np.log(sigmoid(mul_kc))
        z = (1-sigmoid(mul_kc))
        sum2 += z*outsideVectors[i]
        gradOutsideVecs[i] += z*centerWordVec
    mul_oc = np.dot(outsideVectors[outsideWordIdx],centerWordVec)
    loss = -np.log(sigmoid(mul_oc)) - sum1
    gradCenterVec = (sigmoid(mul_oc)-1)*outsideVectors[outsideWordIdx]+sum2
    gradOutsideVecs[outsideWordIdx] = centerWordVec*(sigmoid(mul_oc)-1)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for i in outsideWords:
        ...
        temp_loss, temp_gradC, temp_gradO = word2vecLossAndGradient(centerWordVector,outsideWordIdx,outsideVectors,dataset)
        loss += temp_loss
        gradCenterVecs[centerWordIdx] += temp_gradC
        gradOutsideVectors += temp_gradO
    ### END YOUR CODE
