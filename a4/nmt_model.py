...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        packed_sentences = pack_padded_sequence(X,source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(packed_sentences)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(dec_hidden.unsqueeze(dim=-1).transpose(1,2),enc_hiddens_proj.transpose(1,2)).squeeze(dim=1)
        ###
        ###       Hints:
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(dim=-1).transpose(1,2),enc_hiddens).squeeze(dim=1)
        #$$     Hints:
        ...
        U_t = torch.cat((a_t,dec_hidden.squeeze(dim=-1)),dim=-1)
