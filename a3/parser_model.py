...
class ParserModel(nn.Module):
    ...
    def forward(self, w):
        ...
        x1 = torch.matmul(self.embedding_lookup(w),self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        relu = torch.nn.ReLU()
        logits = torch.matmul(relu(x1),self.hidden_to_logits_weight) + self.hidden_to_logits_bias
